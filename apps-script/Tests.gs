...
var INTERNAL_TESTS_CORE_ = [
  ...
  },
  {
    ...
    run: function () {
      var canonical = buildCanonicalString_({
        version: "1", timestamp: 1700000000000, nonce: "test-nonce", requestId: "test-request-id",
        action: "health", payload: { b: 1, a: 2 },
      });
      assertEqual_(canonical, "1\n1700000000000\ntest-nonce\ntest-request-id\nhealth\n{\"a\":2,\"b\":1}", "canonical string");
      assertEqual_(
        computeHmacHex_(canonical, "test-signing-secret"),
  ...
  {
    name: "computeHmacHex matches Node-generated Vector 2 (nested object, null, array, boolean)",
    run: function () {
      var canonical = buildCanonicalString_({
        version: "1", timestamp: 1700000000000, nonce: "test-nonce-2", requestId: "test-request-id-2",
  ...
  {
    name: "computeHmacHex matches Node-generated Vector 3 (empty payload)",
    run: function () {
      var canonical = buildCanonicalString_({
        version: "1", timestamp: 1700000000000, nonce: "test-nonce-3", requestId: "test-request-id-3",
  ...
  {
    // Reproduces the production INVALID_SIGNATURE bug on createAppointment:
    // customerNotes generated by web-reservas can contain accented Spanish
    // text (e.g. "Género indicado por el cliente: Hombre."). Before
    // Security.gs's computeHmacHex_ passed Utilities.Charset.UTF_8
    // explicitly, this vector's signature diverged from the Next.js side
    // (lib/crm/signing.ts / tests/crm-signing.test.ts Vector 4) for any
    // non-ASCII canonicalString. See SECURITY.md "CRM request signing".
    name: "computeHmacHex matches Node-generated Vector 4 (Unicode customerNotes)",
    run: function () {
      var canonical = buildCanonicalString_({
        version: "1", timestamp: 1700000000000, nonce: "test-nonce-4", requestId: "test-request-id-4",
        action: "createAppointment",
      ...
      assertEqual_(
        ...
        "hmac vector 4 (unicode notes)",
      );
    },
  },
  {
    name: "computeHmacHex matches Node-generated Vector 5 (Unicode customer name: José Núñez)",
    run: function () {
      var canonical = buildCanonicalString_({
        version: "1", timestamp: 1700000000000, nonce: "test-nonce-5", requestId: "test-request-id-5",
        action: "createAppointment",
      ...
      assertEqual_(
        ...
        "hmac vector 5 (unicode name)",
      );
    },
  },
  {
    name: "computeHmacHex matches Node-generated Vector 6 (emoji in customerNotes)",
    run: function () {
      var canonical = buildCanonicalString_({
        version: "1", timestamp: 1700000000000, nonce: "test-nonce-6", requestId: "test-request-id-6",
        action: "createAppointment",
  ...
  {
    name: "computeHmacHex matches Node-generated Vector 7 (newline + accents in customerNotes)",
    run: function () {
      var canonical = buildCanonicalString_({
        version: "1", timestamp: 1700000000000, nonce: "test-nonce-7", requestId: "test-request-id-7",
        action: "createAppointment",
      ...
      assertEqual_(
        ...
        "hmac vector 7 (newline + accents)",
      );
    },
  },
  {
    name: "a signed envelope with Unicode customerNotes passes end-to-end verification",
    run: function () {
      var envelope = buildTestEnvelope_("createAppointment", {
        serviceId: "svc_1",
        barberId: "barber_1",
        localDate: "2026-07-30",
        localStartTime: "11:30",
        customer: { name: "José Núñez", phoneE164: "+59170000000" },
        customerNotes: "Género indicado por el cliente: Hombre. ñ á é í ó ú 💈",
        source: "WEBSITE",
      });
      verifySignedRequest_(envelope); // throws INVALID_SIGNATURE on failure
    },
  },
  {
