...
description: Design, build, connect, test, and deploy booking websites for barbershops (or similar appointment-based small businesses) backed by a shared CRM/booking engine. Use when asked to create a barbershop/salon booking website, connect an existing website to a booking CRM, build a multi-barber scheduling system, prevent double-booking, add an admin dashboard for appointments, add a WhatsApp booking channel, or reuse this architecture for a new client ("create client #N using the same system"). Not for generic e-commerce, restaurants without per-staff scheduling, or one-off static sites with no reservation logic.
---
...
Work through these in order; each has its own invariants and completion criteria detailed
in the references. Don't start a phase's code before its prerequisite decisions/docs exist.
